...
base_income  = st.number_input("Enter your paycheck amount ($)", min_value=0.0, step=10.0)
if freq == "Weekly":
...
if st.checkbox("➕ Add other income sources?"):
    n_other = st.number_input("How many other income sources?", min_value=1, max_value=10, step=1)
    for i in range(int(n_other)):
        label = st.text_input(f"Label for income #{i+1}", key=f"other_label_{i}")
        amt   = st.number_input(f"Amount for {label} ($)", min_value=0.0, step=5.0, key=f"other_amt_{i}")
        monthly_income += amt
...
expenses = {}
def add_expense(cat):
    expenses[cat] = st.number_input(f"{cat} ($)", min_value=0.0, step=5.0, key=cat)
...
if st.checkbox("➕ Add other expenses?"):
    n_exp = st.number_input("How many extra expense categories?", min_value=1, max_value=10, step=1)
    for i in range(int(n_exp)):
        label = st.text_input(f"Label for expense #{i+1}", key=f"exp_label_{i}")
        amt   = st.number_input(f"Amount for {label} ($)", min_value=0.0, step=5.0, key=f"exp_amt_{i}")
        expenses[label] = amt
...
# --- Debts ---
st.header("💳 Debts")
num_debts = st.number_input("How many debts?", min_value=1, max_value=50, step=1)
debts     = []
...
for i in range(int(num_debts)):
    ...
    pay        = c2.number_input(f"Monthly Payment for {name} ($)", min_value=0.0, step=5.0, key=f"pay_{i}")
    owed       = c3.number_input(f"Total Owed on {name} ($)", min_value=0.0, step=5.0, key=f"owed_{i}")
