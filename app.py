...
# --- Debts ---
st.header("💳 Debts")
num_debts = st.number_input("How many debts do you have?", min_value=0, step=1)
debts = []
if num_debts > 0:
...
if not debt_df.empty and len(debt_df) > 1:
    min_payment = debt_df["Monthly Payment"].min()
...
if sum(sizes) > 0:
    ...
    st.warning("No expenses entered to visualize.")
